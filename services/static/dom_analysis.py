...
def _get_agent():
    global _agent
    if _agent is None:
        _agent = make_analyzer("dom_analyzer")
    return _agent
...
async def analyze_dom(
    ...
    """
    Run DOM analyzer (Gemma) on page artifacts.
    ...
    user_msg = json.dumps(
        {
            ...
            "instructions": (
                "Analyze the DOM artifacts. Return the evidence JSON array."
            ),
        }
    )
    ...
    for ev in evidence_list:
        # Build artifact_uris; filter empty strings; fall back to dom_uri.
        raw_uris = ev.get("artifact_uris") or []
        artifact_uris = [u for u in raw_uris if u] or ([dom_uri] if dom_uri else [])
        ...
        ev_doc = {
            ...
            "source": "dom",
            "taxonomy_scope": taxonomy_scope,
            "evidence_type": ev.get("evidence_type", "dom_observation"),
            "url": url,
